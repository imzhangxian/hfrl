...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.critic = nn.Sequential(
            layer_init(nn.Linear(np.array(envs.single_observation_space.shape).prod(), h1_size)),
            nn.Tanh(),
            layer_init(nn.Linear(h1_size, h2_size)),
            nn.Tanh(),
            layer_init(nn.Linear(h2_size, 1), std=1.0),
            # nn.Dropout()
        )
        self.actor = nn.Sequential(
            ...
            layer_init(nn.Linear(h1_size, h2_size)),
            nn.Tanh(),
            layer_init(nn.Linear(h2_size, envs.single_action_space.n), std=0.01),
            # nn.Dropout()
        )
...
if __name__ == "__main__":
    ...
    init_ob, _ = envs.reset()
    next_obs = torch.Tensor(init_ob).to(device)
    ...
    for round in range(eval_rounds):
        ...
        for t in range(eval_max_steps):
            ...
            a, _, _, _ = agent.get_action_and_value(torch.Tensor(s).to(device))
            s, r, done, _, _ = eval_env.step(a.cpu().numpy())
            gain = r + args.gamma * gain
    ...
    # visualize
    video = cv2.VideoWriter('PPO-LunarLander.mp4', cv2.VideoWriter_fourcc(*'mp4v'), 30, (600, 400))
